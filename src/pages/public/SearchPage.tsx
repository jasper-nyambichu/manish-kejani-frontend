...
const SearchPage = () => {
  ...
  // Build query filters from applied state
  ...
  // Derive brand list from current results
  ...
  const hasActiveFilters =
    applied.minPrice || applied.maxPrice || applied.brands.length || applied.rating || applied.condition;
    ...
    <aside className="w-full space-y-5">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {[5, 4, 3, 2, 1].map(star => (
            <label key={star} className="flex items-center gap-2 cursor-pointer group">
              <input
                ...
                onChange={() => setDraft(d => ({ ...d, rating: star }))}
                className="accent-primary w-3.5 h-3.5"
              />
          ...
          {draft.rating > 0 && (
            <button onClick={() => setDraft(d => ({ ...d, rating: 0 }))} className="text-xs text-muted-foreground hover:text-primary">
              Clear
            </button>
          )}
        </div>
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {CONDITIONS.map(c => (
            <label key={c} className="flex items-center gap-2 cursor-pointer group">
              <input
                ...
                onChange={() => setDraft(d => ({ ...d, condition: c }))}
                className="accent-primary w-3.5 h-3.5"
              />
          ...
          {draft.condition && (
            <button onClick={() => setDraft(d => ({ ...d, condition: '' }))} className="text-xs text-muted-foreground hover:text-primary">
              Clear
            </button>
          )}
        </div>
      ...
      <button
        ...
        className="w-full h-9 bg-primary text-primary-foreground text-sm font-semibold rounded-button hover:opacity-90 transition-opacity"
      >
        Apply Filters
      </button>
    </aside>
    ...
    <div className="min-h-screen bg-background font-body">
      ...
      <main>
        ...
        <div className="container mx-auto px-4 pb-8">
          {/* Sort bar */}
          <div className="bg-card rounded-card border border-border p-3 sm:p-4 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
              <div className="min-w-0">
                <h1 className="font-display text-lg sm:text-xl text-foreground truncate">
                  {query ? `Results for "${query}"` : 'Search Products'}
                </h1>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {isLoading
                    ? 'Searching...'
                    : `${pagination.total.toLocaleString()} result${pagination.total !== 1 ? 's' : ''}${query ? ` for ${query}` : ''}`
                  }
                </p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  ...
                  className="lg:hidden flex items-center gap-1.5 h-9 px-3 text-sm border border-border rounded-button hover:bg-secondary transition-colors whitespace-nowrap"
                >
                ...
                <select
                  ...
                  className="flex-1 sm:flex-none text-sm bg-secondary border border-border rounded-button px-2 sm:px-3 py-1.5 text-foreground focus:outline-none focus:ring-1 focus:ring-primary/40 min-w-0"
                >
                  {SORT_OPTIONS.map(o => (
          ...
          <div className="flex gap-5">
            {/* Desktop sidebar — sticky below the full navbar (top-bar + main + category nav ≈ 112px) */}
            ...
            {/* Mobile sidebar overlay — 85vw wide, max 320px, slides from left */}
            {sidebarOpen && (
            ...
            <div className="flex-1 min-w-0">
              {isLoading ? (
                ...
                <div className="bg-card rounded-card border border-border p-8 sm:p-16 text-center">
                  ...
                  {pagination.pages > 1 && (
                    <div className="flex justify-center items-center gap-2 mt-6">
                      <button
                        ...
                        disabled={page === 1}
                        className="h-9 min-w-[80px] px-3 text-sm font-body border border-border rounded-button hover:bg-secondary disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        Previous
                      </button>
                      <span className="h-9 px-3 flex items-center text-sm font-body text-muted-foreground whitespace-nowrap">
                ...
                <div className="bg-card rounded-card border border-border p-8 sm:p-16 text-center">
                  ...
                  {hasActiveFilters && (
                    <button onClick={clearFilters} className="text-primary hover:underline text-sm font-medium mr-4">
                      Clear Filters
                    </button>
                  )}
